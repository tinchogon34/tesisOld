...
post '/data' do
    ...
    slice_id = params[:current_slice]
    results = params[:result] #[["0",[1,2]],["2",[3]]]

    # args are required
    if not (doc_id and current_slice != nil and results != nil)
        return "Wrong arguments"
    end

    # ya se agrego la respuesta?
    coll = settings.db["workers"].find({"_id" => BSON::ObjectId(doc_id)})
    if coll.

    end

    settings.db["workers"].update({"_id" => BSON::ObjectId(doc_id)},
        '$push' => { :map_results => results})
    #   '$push' => { :map_results[slice_id] => results})
    settings.workers = settings.db["workers"].find({"status" => "created"}).to_a
